...
class TestDepending(unittest.TestCase):
	...
	""" Method to verify that the importation order is right """
		
    def test_get_ordre_import(self):

        res_country = depending.get_ordre_importation('admin', 'admin', 'test',
                                                     ('res.country',),
                                                      None, None)
        res_account = depending.get_ordre_importation('admin', 'admin',
                                                      'test',
                                                      ('account.account',),
                                                      None, None)

        res_groups_ex = depending.get_ordre_importation('admin', 'admin',
                                                        'test',
                                                        ('res.groups',),
                                                        ['ir.module.category'],
                                                        None)

        res_groups = depending.get_ordre_importation('admin', 'admin', 'test',
                                                     ('res.groups',), None,
                                                     None)
        ...
        self.assertEquals(res_groups_ex, ['ir.ui.menu', 'account.financial.report',
        ...
        self.assertEquals(res_groups, ['ir.ui.menu', 'account.financial.report',
         'account_account_financial_report_type', 'account.tax.code', 'account.tax',
         'account.account.type', 'account.account', 'account_account_consol_rel',
         'account_account_financial_report', 'account_account_tax_default_rel',
         'res.country.state', 'res.country', 'res.currency', 'res.partner.category',
